...
如果节点可以访问互联网，直接启动chronyd服务即可，CentOS  7自带此服务。
    ...
    ~]# systemctl enable chronyd && systemctl start chronyd</code>
...
一般推荐使用本地的时间服务器，修改/etc/chrony.conf配置文件，将server后配置修改为指定时间服务器地址即可，如果有多个，指定多个server即可，server后面跟时间服务器的地址，如。 
    ...
    server 0.centos.pool.ntp.org iburst</code>
可以使用以下命令可以查看时间同步的信息。 
    
    ~]# chronyc sources -v</code>
### 配置主机名解析
...
为防止防火墙的干扰，推荐将防火墙关闭，操作如下。 
    ...
    ~]# systemctl disable firewalld && systemctl stop firewalld</code>
### 关闭SELinux
同样为了防止SELinux干扰，默认系统是启用了SELinux，需要将此关闭。修改/etc/sysconfig/selinux文件来禁用SELinux，修改配置如下。 
    
    ~]# sed -i "/^SELINUX/s@enforcing@disabled@" /etc/sysconfig/selinux</code>
修改完SELinux配置后需要重启系统。 
## 准备工作
...
使用Ceph官方仓库安装下载较慢，国内阿里云镜像站也提供Ceph相关镜像，地址如下。
    ...
    https://mirrors.aliyun.com/ceph/</code>
安装阿里云Ceph的repo源，在所有节点上安装此源。
    
    ~]# rpm -ivh https://mirrors.aliyun.com/ceph/rpm-mimic/el7/noarch/ceph-release-1-1.el7.noarch.rpm</code>
### 准备普通用户
...
在所有节点上创建用户并设置密码。
    ...
    ~]# useradd cephadmin && echo "cephadmin" | passwd cephadmin --stdin</code>
...
创建文件/etc/sudoers.d/cephadmin，文件内容如下。 
    ...
    ~]# echo "cephadmin ALL = (root) NOPASSWD: ALL" | tee /etc/sudoers.d/cephadmin</code>
更改cephadmin文件的权限。 
    
    ~]# chmod 0440 /etc/sudoers.d/cephadmin</code>
### 部署ceph-deploy
接下来将部署ceph-deploy节点，主要就是安装ceph-deploy软件包及其依赖包。此在administrator节点上面执行即可。
    
    ~]# yum install ceph-deploy python-setuptools python2-subprocess32</code>
## 部署Rados集群
...
2.  创建一个集群配置信息和密钥环境文件等，初化时需要指定集群中一个monitor节点。 
    ...
    ~]$ ceph-deploy new --cluster-network 18.50.129.0/24 --public-network 192.168.6.0/24 ceph-monitor-1 </code>
...
安装ceph集群需要使用ceph-deploy的install子命令，首先初始化所有monitor节点。
    ...
    ~]$ ceph-deploy install ceph-monitor-1 ceph-monitor-2 ceph-monitor-3</code>
接下来初始化所有storage节点，执行下面的命令。
    
    ~]$ ceph-deploy install ceph-storage-1 ceph-storage-2 ceph-storage-3 ceph-storage-4</code>
...
根据前面主机说明，我们将ceph-monitor-1和ceph-monitor-2节点也作为manager节点，所以我们将在ceph-monitor-1部署第一个manager节点 。执行如下命令配置。
    ...
    ~]$ ceph-deploy mgr create ceph-monitor-1</code>
...
默认情况下ceph.client.admin.keyring文件的权限为600，属主和属组为root，如果在集群内节点使用cephadmin用户直接直接ceph命令，将会提示无法找到/etc/ceph/ceph.client.admin.keyring文件，因为权限不足。当然如果使用sudo ceph不存在此问题，为方便直接使用ceph命令，可将权限设置为644。在集群节点上面cephadmin用户下执行下面命令。 
    ...
    ~]$ sudo chmod 644 /etc/ceph/ceph.client.admin.keyring </code>
8.  集群健康状态检查。 
...
1.  列出storage节点上面可用的物理硬盘。 
    ...
    ~]$ ceph-deploy disk list ceph-storage-1</code>
...
2.  擦除指定storage节点，指定硬盘的数据。
    ...
    ~]$ ceph-deploy disk zap ceph-storage-1 /dev/sdb</code>
3.  添加指定storage节点，指定硬盘为OSD，相当于创建一个OSD。
    
    ~]$ ceph-deploy osd create ceph-storage-1 --data /dev/sdb</code>
添加成功后提示如下信息。
...
还可以使用以下命令分别查看指定storage节点上面的OSD信息。
    ...
    ~]$ ceph-deploy osd list ceph-storage-1</code>
...
1.  首先创建一个用于数据存储的存储池。
    ...
    ceph osd pool create testpool 32 32</code>
创建了一个名为testpool的存储池，并指定有32个PG和32个用于归置的PG，第二个代表PGP（归置PG），如果不指定默认与PG相等，推荐相等。
2.  上传文件至指定存储池。
    
    ~]$ rados put fstab /etc/fstab --pool=testpool</code>
...
4.  删除存储池中指定的对象。
    ...
    ~]$ rados rm fstab --pool=testpool</code>
5.  删除存储池。
...
还可以使用quorum_status子命令查看monitor更详细的信息。
    ...
    ~]$ ceph quorum_status --format json-pretty</code>
...
执行下面的命令可添加另一个manager节点。
    ...
    ~]$ ceph-deploy mgr create ceph-monitor-2</code>
创建成功后查看集群状态如下。
...
1.  在管理节点上面执行destroy子命令删除指定的monitor节点。
    ...
    ~]$ ceph-deploy mon destroy ceph-monitor-2</code>
...
服务名为：ceph-osd@<osd id>。
    ...
    ~]$ sudo systemctl stop ceph-osd@0</code>
3.  再使用ceph命令将该OSD从集群中移除。
    
    ~]$ ceph osd purge 0 --yes-i-really-mean-it</code>
注意：此方法不适用于L版本之前的版本。 
### 创建RBD接口
RBD主要是块设备接口，通常RBD为KVM等虚拟化OS提供高并发到有无限可扩展性的后端存储。创建RBD接口需要创建专门用于RBD的存储池，其方法如下。
1.  创建一个普通存储池。
    
    ~]$ ceph osd pool create kvm 32 32</code>
  * kvm：定义一个名为kvm的存储池，并指定pg与pgp都为32。
2.  将存储池转换为RBD模式。
    
    ~]$ ceph osd pool application enable kvm rbd</code>
相当于开启存储池的rbd模式，还有其它模式。
3.  初始化存储池。
    
    ~]$ rbd pool init -p kvm</code>
4.  创建镜像。
    
    ~]$ rbd create img1 --size 10240 --pool kvm</code>
  * img1：指定镜像名称
  * --size：指定镜像大小。
  * --pool：指定创建到哪个pool。
5.  查看镜像相关信息。
...
1.  创建rgw守护进程，可以创建在集群任何节点。
    ...
    ~]$ ceph-deploy rgw create ceph-storage-1</code>
...
1.  使用以下命令创建mds实例，可以创建在集群任何节点，这里根据主机说明创建在ceph-monitor-1节点。
    ...
    ~]$ ceph-deploy mds create ceph-monitor-1</code>
...
备注：告知Ceph集群不要将OSD标记为out，可以使用以下命令。
    ...
    ~]$ ceph osd set noout</code>
2.  启动。
与停止反方向。最后需要告诉Ceph集群可以将OSD标记为out，命令如下。
    
    ~]$ ceph osd unset noout</code>
